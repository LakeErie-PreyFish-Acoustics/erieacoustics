...
sample_grids_proposed <- function(basin,year) {
  ...
  p1 <- eriespatial::base_erieshore +
    ...
    ggplot2::geom_text(data = shape_5mincent_surv_sub, aes(label = GRID, x = X, y = Y),
  ...
  p2 <- eriespatial::base_erieshore +
    ...
    ggplot2::geom_text(data = surv_cent, aes(label = GRID, x = X, y = Y),
  ...
  sample_grids <- surv_cent %>% dplyr::select(BASIN,STRATUM,GRID,Priority,Y,X) %>%
              dplyr::left_join(Effort_Allocation_sub[,c(2,10)], by="STRATUM") %>%
              sf::st_drop_geometry() %>% dplyr::rename(Basin = BASIN) %>%
              dplyr::rename(Stratum = STRATUM) %>% dplyr::rename(Grid = GRID) %>%
              dplyr::rename(Latitude = Y) %>% dplyr::rename(Longitude = X) %>%
